-- @deprecated Tabelas criadas pela versão canônica: 20260101_create_tech_sheet_tables.php
-- Este arquivo é mantido para referência histórica.
-- O runner (bin/migrate.php) executa ambos em ordem; o IF NOT EXISTS garante idempotência.
-- Diferença: completeness_percent aqui era DECIMAL(5,1) — normalizado para DECIMAL(5,2) abaixo.

-- Criar tabelas base (se ainda não existirem — PHP canônico pode ter rodado antes)
...
-- Normalizar DECIMAL(5,1) → DECIMAL(5,2) se veio de versão anterior
ALTER TABLE tech_sheet_item_summary MODIFY COLUMN completeness_percent DECIMAL(5,2) DEFAULT 0.00;
