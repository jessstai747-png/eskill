...
class EanInventory
{
    ...
    public function getAvailable(int $limit = 100): array
    {
        ...
        $stmt = $this->db->prepare("
            SELECT id, ean, status, purchase_batch, cost, supplier, created_at
            FROM ean_inventory
    ...
    public function list(int $page = 1, int $perPage = 50, ?string $status = null, ?string $batch = null): array
    {
        ...
        // Dados
        $sql = "SELECT id, ean, status, purchase_batch, cost, supplier, reserved_at, sold_at, created_at FROM ean_inventory {$whereClause} ORDER BY id DESC LIMIT {$limitSql} OFFSET {$offsetSql}";
