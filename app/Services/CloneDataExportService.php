...
class CloneDataExportService
{
    ...
    public function exportItemsToCsv(array $filters = []): array
    {
        ...
        $filename = 'clone_items_' . date('Y-m-d_His') . '.csv';
        ...
        $this->logExport('csv', $filename, count($items));
    ...
    public function exportItemsToJson(array $filters = []): array
    {
        ...
        $filename = 'clone_items_' . date('Y-m-d_His') . '.json';
        $filepath = $this->exportPath . '/' . $filename;
        ...
        $this->logExport('json', $filename, count($items));
    ...
    public function exportJobsToCsv(array $filters = []): array
    {
        ...
        $filename = 'clone_jobs_' . date('Y-m-d_His') . '.csv';
        $filepath = $this->exportPath . '/' . $filename;
        ...
        $this->logExport('csv', $filename, count($jobs));
    ...
    public function exportFullReport(array $options = []): array
    {
        ...
        $this->logExport('html', $filename, count($items));
...
<body>
     ...
     * Registra exportação
     */
    private function logExport(string $type, string $filename, int $itemCount): void
    {
        try {
            $stmt = $this->db->prepare("
                INSERT INTO clone_export_logs (
                    account_id, export_type, filename, item_count, created_at
                ) VALUES (
                    :account_id, :type, :filename, :count, NOW()
                )
            ");
            $stmt->execute([
                'account_id' => $this->accountId,
