...
class KeywordKiller
{
    ...
    private const BUYING_INTENT_WORDS = [
        'comprar', 'preço', 'barato', 'promoção', 'onde encontrar',
        'melhor', 'original', 'frete grátis', 'entrega rápida',
        'novo', 'lacrado', 'garantia', 'qualidade',
    ];
    ...
    private function generateLongTail(array $productData, array $baseKeywords): array
    {
        ...
        // Clean and unique
        $longTail = array_map(fn($k) => trim(preg_replace('/\s+/', ' ', $k)), $longTail);
...
{\"keywords\": [\"keyword1\", \"keyword2\", ...]}";
    ...
    {
        $stopwords = [
            'de', 'da', 'do', 'das', 'dos', 'para', 'com', 'em', 'por',
            'que', 'uma', 'um', 'seu', 'sua', 'este', 'esta', 'isso',
            'the', 'and', 'for', 'with', 'new', 'set',
        ];
