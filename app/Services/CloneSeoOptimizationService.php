...
class CloneSeoOptimizationService
{
    ...
    private array $forbiddenWords = [
        'réplica', 'replica', 'imitação', 'imitacao', 'fake', 'falso',
        'primeira linha', '1a linha', 'aaa', 'grau a', 'importado china',
        'pronta entrega', 'envio imediato', 'frete grátis', 'frete gratis',
        'promoção', 'promocao', 'oferta', 'liquidação', 'liquidacao',
        'black friday', 'natal', 'ano novo'
    ];
    
    // Palavras de alto impacto para SEO ML
    private array $highImpactWords = [
        'original', 'genuíno', 'genuino', 'autêntico', 'autentico',
        'novo', 'lacrado', 'garantia', 'nf', 'nota fiscal',
        'premium', 'profissional', 'alta qualidade'
    ];
    ...
    private function generateOptimizations(array $analysis, string $level): array
    {
        ...
        if ($level === self::LEVEL_BASIC || $level === self::LEVEL_STANDARD || 
        ...
        if ($level === self::LEVEL_STANDARD || $level === self::LEVEL_ADVANCED || 
