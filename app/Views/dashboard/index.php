...
.metric-icon.primary { background: var(--primary-gradient); }
...
.metric-icon.info { background: linear-gradient(135deg, #06b6d4 0%, #0891b2 100%); }
...
.metric-card[data-color="primary"]::before { --metric-color: var(--primary-gradient); }
.metric-card[data-color="success"]::before { --metric-color: var(--success-gradient); }
.metric-card[data-color="warning"]::before { --metric-color: var(--warning-gradient); }
.metric-card[data-color="danger"]::before { --metric-color: var(--danger-gradient); }
.metric-card[data-color="info"]::before { --metric-color: linear-gradient(135deg, #06b6d4 0%, #0891b2 100%); }
...
.order-icon.paid { background: rgba(34, 197, 94, 0.1); color: #16a34a; }
.order-icon.pending { background: rgba(245, 158, 11, 0.1); color: #d97706; }
.order-icon.shipped { background: rgba(59, 130, 246, 0.1); color: #2563eb; }
.order-icon.cancelled { background: rgba(239, 68, 68, 0.1); color: #dc2626; }
...
.rep-progress-bar.success { background: var(--success-gradient); }
.rep-progress-bar.warning { background: var(--warning-gradient); }
.rep-progress-bar.danger { background: var(--danger-gradient); }
...
function renderAccounts() {
    ...
    grid.innerHTML = accountsState.accounts.map(account => {
        const tokenStatus = accountsState.tokenStatus[account.id] || { token_status: 'unknown' };
...
function getConnectionMeta(account, tokenStatus) {
    ...
    if (accountsState.pendingId === account.id) {
        return { label: 'Ativando', class: 'badge-syncing', icon: 'bi-arrow-repeat', tokenLabel: '...' };
    }
    if (tokenState === 'valid') {
        return { label: 'Conectada', class: 'badge-connected', icon: 'bi-check-circle', tokenLabel: 'Válido' };
    }
    if (tokenState === 'expiring_soon') {
        return { label: 'Expirando', class: 'badge-expiring', icon: 'bi-exclamation-triangle', tokenLabel: 'Expirando' };
    }
    if (tokenState === 'expired') {
        return { label: 'Expirada', class: 'badge-disconnected', icon: 'bi-x-circle', tokenLabel: 'Expirado' };
    }
    return { label: 'Desconhecida', class: 'badge-unknown', icon: 'bi-question-circle', tokenLabel: 'N/A' };
}

function getSyncMeta(syncStatus, accountId) {
    if (accountsState.pendingId && accountsState.pendingId === accountId) {
        return { label: 'Verificando', class: 'badge-syncing', icon: 'bi-arrow-repeat' };
    }
    if (syncStatus.needs_sync === false) {
        return { label: 'Sincronizada', class: 'badge-synced', icon: 'bi-check-circle' };
    }
    if (syncStatus.needs_sync === true) {
        return { label: 'Desincronizada', class: 'badge-unsynced', icon: 'bi-x-circle' };
    }
    return { label: 'Sem Sync', class: 'badge-unknown', icon: 'bi-dash-circle' };
}
...
async function refreshSyncStatuses() {
    const syncRequests = accountsState.accounts.map(account => {
        return requestJson(`/api/accounts/${account.id}/sync/status`)
            .then(data => ({ id: account.id, status: data.data || {} }))
            .catch(() => ({ id: account.id, status: {} }));
    });
...
async function loadOrders() {
    try {
        const data = await requestJson('/api/orders/all?limit=6');
        ...
        list.innerHTML = data.results.map(order => {
            ...
            const statusConfig = {
                paid: { icon: 'bi-check-circle', class: 'paid' },
                confirmed: { icon: 'bi-check', class: 'paid' },
                shipped: { icon: 'bi-truck', class: 'shipped' },
                delivered: { icon: 'bi-house-check', class: 'paid' },
                cancelled: { icon: 'bi-x-circle', class: 'cancelled' },
                pending: { icon: 'bi-clock', class: 'pending' }
            }[status] || { icon: 'bi-clock', class: 'pending' };
...
function updateReputation(rep) {
    ...
    let level = 'Iniciante';
    let color = '#64748b';
    if (clampedScore >= 90) { level = 'MercadoLíder Platinum'; color = '#f59e0b'; }
    else if (clampedScore >= 80) { level = 'MercadoLíder Gold'; color = '#eab308'; }
    else if (clampedScore >= 70) { level = 'MercadoLíder'; color = '#22c55e'; }
    else if (clampedScore >= 50) { level = 'Bom'; color = '#3b82f6'; }
