...
<style>
    ...
    .text-purple { color: var(--ts-purple) !important; }
    ...
    .kpi-card.danger .value { color: var(--ts-danger); }
    .kpi-card.warning .value { color: var(--ts-warning); }
    .kpi-card.success .value { color: var(--ts-success); }
    .kpi-card.purple .value { color: var(--ts-purple); }
    .kpi-card.purple { border-left: 4px solid var(--ts-purple); }
    ...
    .completeness-bar .fill.critical { background: var(--ts-danger); }
    .completeness-bar .fill.warning { background: var(--ts-warning); }
    .completeness-bar .fill.good { background: var(--ts-success); }
    ...
    .seo-score-mini.excellent { background: linear-gradient(135deg, #06d6a0, #38ef7d); color: white; }
    .seo-score-mini.good { background: linear-gradient(135deg, #56ab2f, #a8e063); color: white; }
    .seo-score-mini.warning { background: linear-gradient(135deg, #f7971e, #ffd200); color: #333; }
    .seo-score-mini.critical { background: linear-gradient(135deg, #ef476f, #f45c43); color: white; }
    ...
    .gaps-badge.critical { background: rgba(239,71,111,0.1); color: var(--ts-danger); }
    .gaps-badge.high { background: rgba(255,209,102,0.15); color: #d49a00; }
    .gaps-badge.medium { background: rgba(72,191,227,0.1); color: var(--ts-secondary); }
    ...
    .gap-item.critical { border-left-color: var(--ts-danger); }
    .gap-item.high { border-left-color: var(--ts-warning); }
    ...
    .confidence-badge.high { background: rgba(6,214,160,0.15); color: #059669; }
    .confidence-badge.medium { background: rgba(255,209,102,0.2); color: #d49a00; }
    .confidence-badge.low { background: rgba(239,71,111,0.1); color: var(--ts-danger); }
    
    /* Hidden SEO Attributes Styles */
    .gaps-badge.hidden { background: rgba(138,43,226,0.1); color: #8b2be2; border: 1px solid rgba(138,43,226,0.2); }
    .gap-item.hidden { border-left-color: #8b2be2; }
    ...
    .kpi-card.hidden .value { color: #8b2be2; }
    ...
    .action-dropdown-item.danger { color: var(--ts-danger); }
    .action-dropdown-item.success { color: var(--ts-success); }
    .action-dropdown-item.purple { color: #8b2be2; }
    .action-dropdown-item.info { color: #17a2b8; }
    ...
    .seo-score-circle.excellent { background: linear-gradient(135deg, #06d6a0, #38ef7d); color: white; }
    .seo-score-circle.good { background: linear-gradient(135deg, #56ab2f, #a8e063); color: white; }
    .seo-score-circle.warning { background: linear-gradient(135deg, #f7971e, #ffd200); color: #333; }
    .seo-score-circle.critical { background: linear-gradient(135deg, #ef476f, #f45c43); color: white; }
    ...
    .strategy-score-item:last-child { border-bottom: none; }
    ...
    .strategy-score.high { color: #059669; }
    .strategy-score.medium { color: #d49a00; }
    .strategy-score.low { color: #ef476f; }
    ...
    .strategy-bar .fill { height: 100%; border-radius: 3px; transition: width 0.3s; }
    .strategy-bar .fill.high { background: #06d6a0; }
    .strategy-bar .fill.medium { background: #ffd166; }
    .strategy-bar .fill.low { background: #ef476f; }
    ...
    .attr-missing { color: var(--ts-danger); font-style: italic; }
    .attr-filled { color: var(--ts-success); }
    ...
    .quick-item-card.medium { border-left-color: var(--ts-warning); }
    .quick-item-card.low { border-left-color: var(--ts-secondary); }
    ...
    .quick-total-gaps.medium { color: var(--ts-warning); }
    .quick-total-gaps.low { color: var(--ts-secondary); }
    ...
    .quick-gap-pill.critical { background: rgba(239,71,111,0.15); color: var(--ts-danger); }
    .quick-gap-pill.filter { background: rgba(255,209,102,0.2); color: #d49a00; }
    .quick-gap-pill.hidden { background: rgba(138,43,226,0.1); color: #8b2be2; }
    ...
    .total-gaps-badge.critical { background: rgba(239,71,111,0.15); color: var(--ts-danger); }
    .total-gaps-badge.warning { background: rgba(255,209,102,0.2); color: #d49a00; }
    .total-gaps-badge.good { background: rgba(6,214,160,0.15); color: #059669; }
...
.bulk-seo-char-count.warn { color: #f59e0b; }
.bulk-seo-char-count.danger { color: #dc2626; }
...
const TechSheet = {
            ...
            if (d.success_rate?.chart) {
                new Chart(document.getElementById('sf-chart-success'), {
                    ...
                    options: { 
                        ...
                        scales: { x: { max: 100, title: { display: true, text: 'Taxa de Aceitação (%)' } } }
                    }
                });
            }
    ...
    updatePagination(pagination) {
        const { page = 1, per_page = 20, total = 0, pages = 1 } = pagination;
    ...
    renderDrawer(data) {
        ...
        const regularGaps = [
            ...(gaps.gaps?.required || []).map(g => ({...g, type: 'critical'})),
            ...(gaps.gaps?.filter || []).map(g => ({...g, type: 'high'})),
            ...(gaps.gaps?.recommended || []).slice(0, 5).map(g => ({...g, type: 'low'})),
        ];
    ...
    async decideSuggestion(itemId, attributeId, status) {
        try {
            const data = await requestJson(`/api/seo/technical-sheet/items/${encodeURIComponent(itemId)}/suggestions/decisions`, {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    decisions: [{ attribute_id: attributeId, status: status }]
                })
            });
    ...
    async bulkApprove() {
        ...
        try {
            const data = await requestJson('/api/seo/technical-sheet/batch/approve', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ item_ids: ids, min_confidence: minConfidence })
            });
    ...
    showQualityResults(itemId, data) {
        ...
        const scoreCategories = [
            { key: 'title', icon: 'heading', label: 'Título' },
            { key: 'description', icon: 'align-left', label: 'Descrição' },
            { key: 'images', icon: 'images', label: 'Imagens' },
            { key: 'attributes', icon: 'list-check', label: 'Atributos' },
            { key: 'shipping', icon: 'truck', label: 'Frete' },
            { key: 'pricing', icon: 'tag', label: 'Preço' }
        ];
    ...
    async openExtractModal(itemId) {
        ...
        try {
            ...
            const extractData = await requestJson(`/api/seo/technical-sheet/items/${encodeURIComponent(itemId)}/extract-from-title`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ title, category_id: data.item.category_id })
            });
    ...
    async openCompareModal(itemId) {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.state.currentItem = { item: { item_id: itemId } };
    ...
    renderSEOAnalysis(container, data, itemId) {
        ...
        const strategyItems = [
            { key: 'synonyms', name: 'Sinônimos', icon: '📚' },
            { key: 'hidden_fields', name: 'Campos Ocultos', icon: '🔮' },
            { key: 'search_coverage', name: 'Cobertura Busca', icon: '🔍' },
            { key: 'field_weights', name: 'Pesos Campos', icon: '⚖️' },
            { key: 'use_contexts', name: 'Contextos Uso', icon: '📍' },
            { key: 'long_tail', name: 'Long Tail', icon: '🎯' },
            { key: 'compatibility', name: 'Compatibilidade', icon: '🔗' },
            { key: 'faq', name: 'FAQs', icon: '❓' },
        ].map(s => {
    ...
    async applySEOTitleFromModal() {
        ...
        try {
            ...
            const data = await requestJson(`/api/seo/technical-sheet/items/${encodeURIComponent(itemId)}/apply-optimized-title`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ title, use_generated: false, meta })
            });
    ...
    async applySEODescriptionFromModal() {
        ...
        try {
            ...
            const data = await requestJson(`/api/seo/technical-sheet/items/${encodeURIComponent(itemId)}/apply-optimized-description`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ plain_text, use_generated: false, meta })
            });
    ...
    async applyOptimizedTitleQuick(itemId) {
        ...
        try {
            const data = await requestJson(`/api/seo/technical-sheet/items/${encodeURIComponent(itemId)}/apply-optimized-title`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ use_generated: true, meta: { reason: 'tech_sheet_ui_quick_apply' } })
            });
            if (data.success) {
                const versionId = data.version_id ? ` (v${data.version_id})` : '';
                this.showToast(`✅ Título aplicado${versionId}`);
    ...
    renderSEOHistory(container, rows, itemId, focusRowId = null) {
        ...
        const fmtType = (t) => {
            const map = { title: 'Título', description: 'Descrição', attributes: 'Atributos', images: 'Imagens', price: 'Preço' };
    ...
    async rollbackSEO(itemId, versionId) {
        ...
        try {
            const data = await requestJson(`/api/seo/technical-sheet/items/${encodeURIComponent(itemId)}/rollback/${encodeURIComponent(versionId)}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ reason: 'Rollback via Ficha Técnica' })
            });
