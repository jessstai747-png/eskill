...
/* Score colors */
.score-critical { --score-color: #ef4444; }
.score-warning { --score-color: #f59e0b; }
.score-good { --score-color: #22c55e; }
.score-great { --score-color: #06b6d4; }

.score-circle .score-fill { stroke: var(--score-color); }
.score-value { color: var(--score-color); }
...
.price-diff-neutral { color: #6b7280; }
...
.attention-card.severity-critical { background: #fef2f2; border: 1px solid #fecaca; }
.attention-card.severity-warning { background: #fffbeb; border: 1px solid #fde68a; }
...
.rep-level-5 { background: #dcfce7; color: #166534; }
.rep-level-4 { background: #d1fae5; color: #065f46; }
.rep-level-3 { background: #fef9c3; color: #854d0e; }
.rep-level-2 { background: #fed7aa; color: #9a3412; }
.rep-level-1 { background: #fecaca; color: #991b1b; }
...
.timing-badge .bi { font-size: 0.65rem; }
...
.export-dropdown-menu button:hover { background: #f0f0f0; }
[data-theme="dark"] .export-dropdown-menu { background: #2b2b2b; }
[data-theme="dark"] .export-dropdown-menu button { color: #ddd; }
[data-theme="dark"] .export-dropdown-menu button:hover { background: #3a3a3a; }
...
@media print {
    .health-hero { break-inside: avoid; }
    .pillar-card { break-inside: avoid; }
    .action-item { break-inside: avoid; }
    .btn, .sidebar, .bottom-nav, .navbar, .btn-group { display: none !important; }
    .refresh-timer, #quickActionsPanel, .auto-refresh-badge { display: none !important; }
    .card { border: 1px solid #ddd !important; box-shadow: none !important; }
    .health-hero { background: #f8f9fa !important; color: #333 !important; }
    .health-hero * { color: #333 !important; }
    .section-header .btn { display: none !important; }
    .score-goal-container, .kbd-hint, .btn-complete { display: none !important; }
    .section-lazy { opacity: 1 !important; transform: none !important; }
    .timing-badge, .retry-toast, .offline-banner, .export-dropdown-menu { display: none !important; }
    body { font-size: 11pt; }
}
...
@media (max-width: 576px) {
    ...
    .export-dropdown-menu { left: 0; right: 0; }

    /* Timing badge - smaller */
    .timing-badge { font-size: 0.6rem; padding: 0.1rem 0.35rem; }

    /* Score goal - stack vertically */
    .score-goal-container { flex-wrap: wrap; justify-content: center; }

    /* Action items - tighter spacing */
    .action-item { padding: 0.75rem; }
    .action-item .d-flex { flex-wrap: wrap; gap: 0.3rem; }
    ...
    .section-header h5 { font-size: 0.95rem; }
    .section-header .badge { font-size: 0.65rem; }

    /* Offline banner - safe area */
    .offline-banner { padding-top: env(safe-area-inset-top, 0.4rem); }
}
...
.loading-step.active { color: #f59e0b; }
.loading-step.pending { color: #94a3b8; }
...
.pillar-delta.delta-up { background: rgba(34, 197, 94, 0.15); color: #22c55e; }
.pillar-delta.delta-down { background: rgba(239, 68, 68, 0.15); color: #ef4444; }
.pillar-delta.delta-same { background: rgba(156, 163, 175, 0.15); color: #9ca3af; }
.hero-delta {
    font-size: 0.85rem;
    font-weight: 600;
    margin-top: 0.25rem;
...
.hero-delta.delta-up { color: #22c55e; }
.hero-delta.delta-down { color: #ef4444; }
...
.weekly-plan-action:hover { background: rgba(0,0,0,0.02); }
[data-theme="dark"] .weekly-plan-action { border-color: rgba(255,255,255,0.06); }
[data-theme="dark"] .weekly-plan-action:hover { background: rgba(255,255,255,0.03); }
.weekly-plan-number {
    width: 28px;
    height: 28px;
    border-radius: 50%;
...
[data-theme="dark"] .category-row { border-color: rgba(255,255,255,0.06); }
.category-issue-bar {
    width: 80px;
    height: 6px;
    background: #e5e7eb;
...
.recovery-badge.reactivate { background: #dcfce7; color: #166534; }
.recovery-badge.optimize { background: #fef9c3; color: #854d0e; }
.recovery-badge.restock { background: #fee2e2; color: #991b1b; }
.recovery-badge.review { background: #e5e7eb; color: #374151; }
[data-theme="dark"] .recovery-badge.reactivate { background: rgba(34,197,94,0.2); color: #86efac; }
[data-theme="dark"] .recovery-badge.optimize { background: rgba(245,158,11,0.2); color: #fcd34d; }
[data-theme="dark"] .recovery-badge.restock { background: rgba(239,68,68,0.2); color: #fca5a5; }
[data-theme="dark"] .recovery-badge.review { background: rgba(156,163,175,0.2); color: #d1d5db; }
    ...
    <div class="col-12 col-sm-6 col-lg">
        <div class="card pillar-card h-100">
            <div class="card-body">
                ...
                <div class="pillar-score-bar"><div class="skeleton" style="width:60%;height:100%;"></div></div>
            </div>
        </div>
    </div>
    <?php endfor; ?>
...
function hideLoadingBar() {
    ...
    if (barContainer) {
        ...
        setTimeout(() => { barContainer.style.display = 'none'; }, 300);
    }
}
...
function toggleActionComplete(actionKey, el) {
    ...
    try {
        const data = {};
        completedActions.forEach(k => { data[k] = Date.now(); });
        localStorage.setItem(COMPLETED_KEY, JSON.stringify(data));
    } catch (e) { /* localStorage indisponível */ }
...
function renderWorstItems(items) {
    ...
    items.forEach(item => {
        ...
        if (item.title_score !== undefined) {
            const bars = [
                { label: 'Tít', val: item.title_score || 0, max: 20, color: '#0d6efd' },
                { label: 'Img', val: item.images_score || 0, max: 20, color: '#198754' },
                { label: 'Atr', val: item.attributes_score || 0, max: 20, color: '#ffc107' },
            ];
...
function renderPriceAnalysis(details) {
    ...
    const factors = [
        { label: 'Tipo Anúncio', score: details.avg_listing_type_score || 0, max: 15, icon: 'bi-star', color: 'primary' },
        { label: 'Frete Grátis', score: details.avg_shipping_score || 0, max: 15, icon: 'bi-truck', color: 'success' },
        { label: 'Logística', score: details.avg_logistics_score || 0, max: 15, icon: 'bi-box-seam', color: 'info' },
        { label: 'Saúde ML', score: details.avg_health_score || 0, max: 15, icon: 'bi-heart-pulse', color: 'danger' },
        { label: 'Catálogo', score: details.avg_catalog_score || 0, max: 10, icon: 'bi-book-half', color: 'primary' },
        { label: 'Qualidade', score: details.avg_quality_score || 0, max: 15, icon: 'bi-image', color: 'warning' },
        { label: 'Vendas', score: details.avg_sales_score || 0, max: 10, icon: 'bi-cart', color: 'success' },
    ];
...
function renderStaleListings(staleData) {
    ...
    if (Object.keys(causes).length > 0) {
        ...
        const causeLabels = {
            tipo_anuncio: { label: 'Tipo anúncio fraco', icon: 'bi-star', color: 'warning' },
        ...
        Object.entries(causes).forEach(([key, count]) => {
            const info = causeLabels[key] || { label: key, icon: 'bi-question-circle', color: 'muted' };
...
function renderOperationPanel(details, score) {
    ...
    const opsBarFactors = [
        { key: 'delay', icon: 'bi-clock', color: 'info' },
        { key: 'questions', icon: 'bi-chat-dots', color: 'primary' },
        { key: 'shipping', icon: 'bi-truck', color: 'success' },
        { key: 'orders', icon: 'bi-x-circle', color: 'danger' },
        { key: 'claims', icon: 'bi-shield-exclamation', color: 'warning' },
        { key: 'compliance', icon: 'bi-clipboard-check', color: 'secondary' },
    ];
...
function renderCrossInsights(insights) {
    ...
    insights.forEach(insight => {
        const pillarsHtml = (insight.pillars || []).map(p => {
            const names = { reputation: 'Reputação', seo_quality: 'SEO', competitiveness: 'Competitividade', operation: 'Operação', sales: 'Vendas' };
    ...
    if (ctxRev) {
        ...
        salesRevenueChartInstance = new Chart(ctxRev, {
            ...
            options: {
                ...
                plugins: {
                    legend: { display: false },
                    tooltip: { callbacks: { label: (ctx) => 'R$ ' + formatPrice(ctx.parsed.y) } }
                },
                scales: {
                    y: { beginAtZero: true, grid: { color: 'rgba(0,0,0,0.05)' }, ticks: { callback: (v) => 'R$ ' + (v >= 1000 ? (v/1000).toFixed(0) + 'k' : v) } },
                    x: { grid: { display: false } }
                }
            }
        });
...
function renderReputationPanel(details) {
    ...
    if (details.power_seller) {
        const psLabels = { platinum: 'MercadoLíder Platinum', gold: 'MercadoLíder Gold', silver: 'MercadoLíder' };
...
function getMLLevelInfo(levelId) {
    if (!levelId) return { label: 'Desconhecido', icon: 'bi-question-circle', cssClass: 'rep-level-1' };

    if (levelId.includes('5_green')) return { label: 'Verde (Nível 5)', icon: 'bi-shield-fill-check', cssClass: 'rep-level-5' };
    if (levelId.includes('4_light')) return { label: 'Verde Claro (Nível 4)', icon: 'bi-shield-check', cssClass: 'rep-level-4' };
    if (levelId.includes('3_yellow')) return { label: 'Amarelo (Nível 3)', icon: 'bi-shield-exclamation', cssClass: 'rep-level-3' };
    if (levelId.includes('2_orange')) return { label: 'Laranja (Nível 2)', icon: 'bi-shield', cssClass: 'rep-level-2' };
    return { label: 'Vermelho (Nível 1)', icon: 'bi-shield-x', cssClass: 'rep-level-1' };
}
...
function showPillarDetail(pillarKey) {
    ...
    if (pillarKey === 'seo_quality') {
        ...
        const levelInfo = details.level_id ? getMLLevelInfo(details.level_id) : { label: 'Desconhecido', icon: 'bi-question-circle', cssClass: '' };
        const psLabels = { platinum: 'MercadoLíder Platinum', gold: 'MercadoLíder Gold', silver: 'MercadoLíder' };
        ...
        const repFactors = [
            { key: 'level',         icon: 'bi-award',           color: 'success' },
            { key: 'power_seller',  icon: 'bi-trophy',          color: 'warning' },
            { key: 'claims',        icon: 'bi-exclamation-triangle', color: 'danger' },
            { key: 'cancellations', icon: 'bi-x-circle',        color: 'danger' },
            { key: 'delays',        icon: 'bi-clock',           color: 'info' },
        ];
    ...
    if (salesUnitsChartInstance) { salesUnitsChartInstance.destroy(); salesUnitsChartInstance = null; }
    if (salesRevenueChartInstance) { salesRevenueChartInstance.destroy(); salesRevenueChartInstance = null; }
...
function renderWeeklyPlan(plan) {
    ...
    plan.actions.forEach((action, idx) => {
        ...
        const pillarNames = {
            reputation: 'Reputação', seo_quality: 'SEO/Qualidade',
            competitiveness: 'Competitividade', operation: 'Operação', sales: 'Vendas'
        };
...
function showFullHistoryChart() {
    ...
    const pillarConfig = [
        { key: 'overall_score', label: 'Geral', color: '#6366f1' },
        { key: 'reputation_score', label: 'Reputação', color: '#22c55e' },
        { key: 'seo_quality_score', label: 'SEO/Qualidade', color: '#3b82f6' },
        { key: 'competitiveness_score', label: 'Competitividade', color: '#f59e0b' },
        { key: 'operation_score', label: 'Operação', color: '#8b5cf6' },
        { key: 'sales_score', label: 'Vendas', color: '#ec4899' }
    ];
...
function renderTrendSparkline(data) {
    ...
    sparklineChartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { display: false },
                y: { display: false, min: Math.max(0, Math.min(...scores) - 10), max: 100 }
            },
            interaction: { mode: 'index', intersect: false }
        }
    });
...
function exportDiagnostic() {
    ...
    if (stale && stale.summary && stale.summary.total_stale > 0) {
        ...
        if (Object.keys(staleCauses).length > 0) {
            const causeNames = { tipo_anuncio: 'Tipo anúncio fraco', sem_frete_gratis: 'Sem frete grátis', saude_baixa: 'Saúde baixa', fora_catalogo: 'Fora do catálogo' };
            text += '  Causas prováveis:\n';
            Object.entries(staleCauses).forEach(([k, v]) => { text += `    - ${causeNames[k] || k}: ${v} anúncios\n`; });
        }
        text += '\n  Lista dos anúncios:\n';
...
function getSeverityLabel(severity) {
    return { 'critical': 'CRÍTICO', 'warning': 'ALERTA', 'info': 'DICA' }[severity] || severity;
}

function getQuickActionLink(actionType) {
    const links = {
        'poor_titles':            { url: '/seo', label: 'Otimizar SEO', icon: 'bi-stars' },
        'few_images':             { url: '/dashboard/items', label: 'Ver Anúncios', icon: 'bi-image' },
        'no_free_shipping':       { url: '/dashboard/items', label: 'Ver Anúncios', icon: 'bi-truck' },
        'missing_attributes':     { url: '/dashboard/seo/ficha-tecnica', label: 'Ficha Técnica', icon: 'bi-card-checklist' },
        'weak_descriptions':      { url: '/seo', label: 'Otimizar', icon: 'bi-text-paragraph' },
        'overpriced':             { url: '/dashboard/items', label: 'Ajustar Preços', icon: 'bi-tag' },
        'unanswered_questions':   { url: '/dashboard/questions', label: 'Responder', icon: 'bi-chat-dots' },
        'no_fulfillment':         { url: 'https://www.mercadolivre.com.br/gz/shipping/preferences', label: 'Config. Frete', icon: 'bi-truck' },
        'sales_declining':        { url: '/dashboard/analytics', label: 'Analytics', icon: 'bi-bar-chart-line' },
        'low_conversion':         { url: '/seo', label: 'Otimizar SEO', icon: 'bi-stars' },
        'low_visibility':         { url: '/seo', label: 'Otimizar SEO', icon: 'bi-stars' },
        'no_active_items':        { url: '/dashboard/items', label: 'Criar Anúncio', icon: 'bi-plus-circle' },
        'high_claims':            { url: '/dashboard/orders', label: 'Ver Pedidos', icon: 'bi-exclamation-triangle' },
        'high_cancellations':     { url: '/dashboard/orders', label: 'Ver Pedidos', icon: 'bi-x-circle' },
        'delayed_shipping':       { url: '/dashboard/orders', label: 'Ver Pedidos', icon: 'bi-clock-history' },
        'low_positive_ratings':   { url: '/dashboard/orders', label: 'Gerenciar', icon: 'bi-star-half' },
        'stale_listings':         { url: '/dashboard/items', label: 'Ver Anúncios', icon: 'bi-archive' },
        'low_health':             { url: '/dashboard/items', label: 'Ver Anúncios', icon: 'bi-heart-pulse' },
        'not_gold_pro':           { url: '/dashboard/items', label: 'Upgrade Tipo', icon: 'bi-star' },
        'not_in_catalog':         { url: '/dashboard/items', label: 'Ver Anúncios', icon: 'bi-book' },
        'incomplete_specs':       { url: '/dashboard/seo/ficha-tecnica', label: 'Ficha Técnica', icon: 'bi-list-check' },
        'ml_quality_low':         { url: '/seo', label: 'Otimizar', icon: 'bi-patch-check' },
        'zero_sales':             { url: '/dashboard/items', label: 'Ver Anúncios', icon: 'bi-graph-down' },
    };
